...
> Let $G = \text{SL}_{d}(\mathbb{Z}_{p})$ be a compact matrix Lie group. Let $A \subset G$ be a finite, topologically generating set, such that $A = A^{-1}$. Furthermore, define the surjective map $G \twoheadrightarrow \text{SL}_{d}(\mathbb{Z}/p^n\mathbb{Z})$ to have kernel $\Gamma_{n} = \text{Ker}(G \twoheadrightarrow \text{SL}_{d}(\mathbb{Z}/p^n\mathbb{Z})) = \{g \in G \mid g \equiv I \pmod{p^{n}}\}$. Let there be a metric (possibly a left-invariant [Finsler metric](https://en.wikipedia.org/wiki/Finsler_manifold)) $d: G \times G \rightarrow \mathbb{R}_{\geq 0}$ defined such that $d(g,h) < 2^{-n}$ if and only if $g^{-1}h \in \Gamma_{n}$. Furthermore, denote each quotient $G_{n} \cong G/\Gamma_{n}$. Lastly, for $\omega \in \langle A \rangle$, define the length function of a word to be $l(\omega) = \min\{n \mid \omega = s_{1}\cdots s_{n}, s_{i} \in A\}$.
> 
><br>
>
